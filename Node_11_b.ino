...
#define READ_TEMP_INTERVAL 100099 //frequency of temperature value update (ms)
#define READ_POS_INTERVAL 20099 //frequency of position value update (ms)
#define READ_LOAD_INTERVAL 123499 //frequency of load value update (ms)
...
void loop() {
  ...
  if(EMERGENCY == 0){ // COMMON STATE (NOT EMERGENCY)
    ...
    if (msg_data_bus1.isreceived) {
      ...
      cmd_servo_bus1.isprocessed = 0;  //AAAAAAAAAAAAAAA ISSO NAO SERVE PARA NADA !!!!!!!!
    ...
    //Verify data coherency in order to leave emergency state
    if(apply_redundancy() == 1 && (EMERGENCY == 1 || EMERGENCY == 3)){
